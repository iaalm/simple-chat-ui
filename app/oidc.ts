...
export const storeTokenSet = (tokenSet: {
  access_token: string;
...
export const refreshAccessToken = async (): Promise<string | null> => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      throw new Error(
        `Token refresh failed: ${errorData.error || tokenResponse.status}`,
      );
    }
...
export const getOIDCConfig = () => {
  return {
    ...
    scope:
      process.env.NEXT_PUBLIC_OIDC_SCOPE || "openid offline_access profile",
    audience:
